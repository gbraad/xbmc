...
#include "cores/RetroPlayer/rendering/VideoRenderers/RPRendererOpenGLESPRIME.h"
...
{
  if (!CRPRendererOpenGLESPRIME::SupportsScalingMethod(renderSettings.GetScalingMethod()))
    return false;
...
bool CRenderBufferPoolGbm::ConfigureInternal()
{
  switch (m_format)
  {
    ...
    {
      m_fourcc = GBM_FORMAT_ARGB8888;
      m_bpp = sizeof(uint32_t);
      return true;
    }
    case AV_PIX_FMT_RGB555:
    {
      m_fourcc = GBM_FORMAT_RGBA5551;
      m_bpp = sizeof(uint16_t);
      return true;
    }
    case AV_PIX_FMT_RGB565:
    {
      m_fourcc = GBM_FORMAT_RGB565;
      m_bpp = sizeof(uint16_t);
      return true;
    }
    default:
