...
CStereoscopicsManager::CStereoscopicsManager(CSettings &settings,
                                             ...
                                             CRenderSystemBase &renderSystem) :
  m_settings(settings),
...
void CStereoscopicsManager::SetStereoMode(const RENDER_STEREO_MODE &mode)
{
  ...
  {
    if (m_renderSystem.SupportsStereo(applyMode))
...
RENDER_STEREO_MODE CStereoscopicsManager::GetNextSupportedStereoMode(const RENDER_STEREO_MODE &currentMode, int step) const
{
  ...
  do
  {
    ...
    if (m_renderSystem.SupportsStereo(mode))
      break;
...
RENDER_STEREO_MODE CStereoscopicsManager::GetStereoModeByUserChoice(const std::string &heading /* = "" */) const
{
  ...
  for (int i = RENDER_STEREO_MODE_OFF; i < RENDER_STEREO_MODE_COUNT; i++)
  {
    ...
    if (m_renderSystem.SupportsStereo(selectableMode))
    {
  ...
  case STEREOSCOPIC_PLAYBACK_MODE_ASK: // Ask
    {
      ...
      if (playing != RENDER_STEREO_MODE_OFF && playing != preferred && preferred != RENDER_STEREO_MODE_AUTO && m_renderSystem.SupportsStereo(playing)) // same as movie
