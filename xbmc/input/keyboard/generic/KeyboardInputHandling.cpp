...
bool CKeyboardInputHandling::OnKeyPress(const CKey& key)
{
  ...
  if (m_buttonMap->GetFeature(source, keyName))
    bHandled = m_handler->OnKeyPress(keyName, static_cast<Modifier>(key.GetModifiers()), key.GetUnicode());
...
void CKeyboardInputHandling::OnKeyRelease(const CKey& key)
{
  ...
  KeyName keyName;
  if (m_buttonMap->GetFeature(source, keyName))
    m_handler->OnKeyRelease(keyName, static_cast<Modifier>(key.GetModifiers()), key.GetUnicode());
}
